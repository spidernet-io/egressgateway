...
<img src="./proposal/01-egress-gateway/Egress Gateway.png" width="80%"></img>

Starting with 2021, we received some feedback as follows.

There are two clusters A and B. Cluster A is VMware-based and runs mainly Database workloads,
and Cluster B is a Kubernetes cluster. Some applications in Cluster B need to access the database in Cluster A,
and the network administrator wants the cluster Pods to be managed through an egress gateway.
...
* status
    * `nodeList` field is the list of nodes matched by `nodeSelector`
        * `ready` field represents the node status, which may be `Ready`, `NotReady` or `Unknown`.
...
* status
    ...
    * `vxlanIPv4` field represents the IPv4 address of VXLAN tunnel.
    * `vxlanIPv6` field represents the IPv6 address of VXLAN tunnel.
...
* spec
    ...
    * `destSubnet` destination CIDR block list.


## Datapath

<img src="./proposal/01-egress-gateway/Egress Gateway Datapath.png" width="80%"></img>
...
<img src="./proposal/01-egress-gateway/Controller Reconcile Flow.png" width="80%"></img>
...
<img src="./proposal/01-egress-gateway/Agent Reconcile Flow.png" width="80%"></img>

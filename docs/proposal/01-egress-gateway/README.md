...
* status
    ...
    * `nodeList` field is the list of nodes matched by `nodeSelector`
        ...
        * `avtive` field represents that the non-egress gateway node is reconcile or reconcile completes accessing the destination CIDR(_e.g. Cluster A CIDR in picture 1_) with this node.
...
status:
  ...
  vxlanIPv6IP: "172.31.0.11/16"
  vxlanIPv4Mac: "xx:xx:xx:xx:xx"
  vxlanIPv6Mac: "xx:xx:xx:xx:xx"
...
* status
    ...
    * `vxlanIPv4Mac` field represents the MAC address of IPv4 VXLAN tunnel Interface.
    * `vxlanIPv6Mac` field represents the MAC address of IPv6 VXLAN tunnel Interface.

...
1. 可使用如下命令快速安装 egressgateway
    ...
    * egressgateway controller 支持高可用，可设置 --set controller.replicas=2
...
3. 任何功能配置，可通过调整 egressgateway 实例租户下的 configmap egressgateway 来实现
...
3. 查看 EgressPolicy 的状态
    ...
    如上输出中：
    * status.eip 展示了该组应用出集群时使用的出口IP 地址

...
func (d *MockDataplane) NewCmd(name string, arg ...string) cmdshim.Command {
	var cmd cmdshim.Command
	d.CmdNames = append(d.CmdNames, name)

	if d.NftablesMode && name != "iptables" {
		if !strings.Contains(name, "-nft") {
			panic("name not contains nft")
		}
	}
	...
	case "iptables-restore", "ip6tables-restore",
		...
		if !reflect.DeepEqual(arg, []string{"--noflush", "--verbose"}) {
			panic("arg must be equal {\"--noflush\", \"--verbose\"})")
		}
		cmd = &restoreCmd{
			Dataplane: d,
		}
	case "iptables-save", "ip6tables-save",
		...
		if !reflect.DeepEqual(arg, []string{"-t", d.Table}) {
			panic("arg must be equal {\"-t\", \" d.Table\"})")
		}
		cmd = &saveCmd{
			Dataplane: d,
		}
	case "iptables":
		if len(arg) != 1 || arg[0] != "--version" {
			panic("arg must be '--version'")
		}
		cmd = &versionCmd{
			Dataplane: d,
		}
	default:
		panic(fmt.Sprintf("Unexpected command %v", name))
	}
...
func (d *restoreCmd) Run() error {
	...
	if d.Dataplane.OnPreRestore != nil {
		fmt.Println("OnPreRestore set, calling it")
		d.Dataplane.OnPreRestore()
		d.Dataplane.OnPreRestore = nil
	}
	if d.Dataplane.FailNextRestore {
		fmt.Println("Simulating an iptables-restore failure")
	...
	for _, line := range lines {
		...
		if strings.HasPrefix(line, "*") {
			// Start of a table.
			if tableSeen {
				if !d.Dataplane.NftablesMode {
					panic("only nft mode should use more than one transaction")
				}
				// We've already had one transaction, check that it was committed
				if !commitSeen {
					panic("commitSeen must be true")
				}
				commitSeen = false
			}
			if line[1:] != d.Dataplane.Table {
				panic("line[1:] != d.Dataplane.Table")
			}
			tableSeen = true
			continue
		}
		if !tableSeen {
			panic("No *table stanza before starting input")
		}
		if commitSeen {
			panic("Unexpected line after COMMIT")
		}
		if line == "COMMIT" {
			commitSeen = true
			continue
		...
		if strings.HasPrefix(line, ":") {
			...
			chainName := parts[0]
			if !reflect.DeepEqual(parts[1:], []string{"-", "-"}) {
				panic("must parts[1:] == []string{\"-\", \"-\"}")
			}
		...
		case "-A", "--append":
			...
			if strings.HasPrefix(chainName, "cali") && d.Dataplane.NftablesMode {
				if d.Dataplane.FlushedChains.Contains(chainName) != true {
					panic("In nft mode, it's not safe to modify chain without flushing")
				}
			}
		...
		case "-I", "--insert":
			...
			rest := strings.Join(parts[2:], " ")
			if chains[chainName] == nil {
				panic("Insert to unknown chain: " + chainName)
			}
		...
		case "-R", "--replace":
			if d.Dataplane.NftablesMode {
				fmt.Println("Replace shouldn't be used in nft mode")
			}
			chainName = parts[1]
			...
			chain := chains[chainName]
			if len(chain) <= ruleIdx {
				panic("replace of non-existent rule")
			}
			chain[ruleIdx] = rest
		...
		case "-D", "--delete":
			...
			if ruleNum, err := strconv.Atoi(parts[2]); err == nil {
				if len(parts) != 3 {
					panic("Unexpected argument after rule position in --delete")
				}

				if !strings.HasPrefix(chainName, "EGRESSGATEWAY") {
					panic("Deleting rule from non-calico chain by number can cause races")
				}

				ruleIdx := ruleNum - 1 // 0-indexed array index of rule.
				chain := chains[chainName]
				if len(chain) <= ruleIdx {
					panic("delete of non-existent rule")
				}
		...
		case "-X", "--delete-chain":
			chainName = parts[1]
			if len(parts) != 2 {
				panic("--delete-chain only has one argument")
			}
			if len(chains[chainName]) != 0 {
				fmt.Println("Only empty chains can be deleted")
			}
...
func (d *saveCmd) StdoutPipe() (io.ReadCloser, error) {
	...
	if d.stdoutPipe != nil {
		panic("StdoutPipe() called more than once")
	}
	d.stdoutPipe = cb
	return cb, nil
}

...
func (b *RestoreInputBuilder) StartTransaction(tableName string) {
	if b.currentTableName != "" {
		panic("StartTransaction() called without ending previous transaction.")
	}
...
func (b *RestoreInputBuilder) EndTransaction() {
	if b.currentTableName == "" {
		panic("EndTransaction() called without active transaction.")
	}
...
func (b *RestoreInputBuilder) writeFormattedLine(format string, args ...interface{}) {
	...
	if err != nil {
		panic(fmt.Sprintf("Failed to write to in-memory buffer: %v", err))
	}
	b.buf.WriteString("\n")
...
func (b *RestoreInputBuilder) maybeWriteTransactionOpener() {
	if b.currentTableName == "" {
		panic("maybeWriteTransactionOpener() called without active transaction.")
	}
	if !b.txnOpenerWritten {
...
// WriteForwardReference writes a "forward reference" for the given chain name.
// A forward reference is an instruction that tells iptables to ensure that the
// given chain exists and that it is empty. Panics if there is no open transaction.
...
// WriteLine writes a line of iptables instructions to a buffer. It should be used to
...
func (b *RestoreInputBuilder) GetBytesAndReset() []byte {
	if b.currentTableName != "" {
		panic("GetBytesAndReset() called inside transaction.")
	}

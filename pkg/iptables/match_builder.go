...
func (m MatchCriteria) MarkSingleBitSet(mark uint32) MatchCriteria {
	if bits.OnesCount32(mark) != 1 {
		...
		// 0-bits too
		panic(fmt.Sprintf("MarkSingleBitSet() should only be used with a single mark bit: %v", mark))
	}
...
func (m MatchCriteria) MarkMatchesWithMask(mark, mask uint32) MatchCriteria {
	...
	if mark&mask != mark {
		panic("Bug: mark is not contained in mask")
	}
...
func (m MatchCriteria) SourcePortRanges(ports []*PortRange) MatchCriteria {
	portsString := PortRangesToMultiPort(ports)
	return append(m, fmt.Sprintf("-m multiport --source-ports %s", portsString))
}

func (m MatchCriteria) NotSourcePortRanges(ports []*PortRange) MatchCriteria {
	portsString := PortRangesToMultiPort(ports)
	return append(m, fmt.Sprintf("-m multiport ! --source-ports %s", portsString))
}

func (m MatchCriteria) DestPortRanges(ports []*PortRange) MatchCriteria {
	portsString := PortRangesToMultiPort(ports)
	return append(m, fmt.Sprintf("-m multiport --destination-ports %s", portsString))
}

func (m MatchCriteria) NotDestPortRanges(ports []*PortRange) MatchCriteria {
	portsString := PortRangesToMultiPort(ports)
	return append(m, fmt.Sprintf("-m multiport ! --destination-ports %s", portsString))
}
...
func (m MatchCriteria) CTDirectionOriginal(direction Direction) MatchCriteria {
	return append(m, fmt.Sprintf("-m conntrack --ctdir %s", direction))
}
...
func PortRangesToMultiPort(ports []*PortRange) string {
	portFragments := make([]string, len(ports))
	for i, port := range ports {

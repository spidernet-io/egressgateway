...
type Interface interface {
	...
	// TestEntry test if an entry exists in the named set
...
func (set *IPSet) Validate() (bool, error) {
	...
	if set.SetType == HashIPPort || set.SetType == HashIPPortIP || set.SetType == HashIPPortNet || set.SetType == HashNet {
		if valid, err := validateHashFamily(set.HashFamily); !valid {
			return false, err
		}
	}
	// check set type
	if valid, err := validateIPSetType(set.SetType); !valid {
		return false, err
	}
	...
	if set.HashSize <= 0 {
		return false, fmt.Errorf("invalid hashsize value %d, should be >0", set.HashSize)
	}
	// check max elem value of ipset
	if set.MaxElem <= 0 {
		return false, fmt.Errorf("invalid maxelem value %d, should be >0", set.MaxElem)
	}
...
func (e *Entry) Validate(set *IPSet) (bool, error) {
	if e.Port < 0 {
		return false, fmt.Errorf("entry %v port number %d should be >=0 for ipset %v", e, e.Port, set)
	}
	switch e.SetType {
	case HashIPPort:
	...
	case HashIPPortNet:
		...
		if _, ipNet, err := net.ParseCIDR(e.Net); ipNet == nil {
			return false, fmt.Errorf("error parsing entry %v ip net %v for ipset %v, error: %v", e, e.Net, set, err)
		}
	case BitmapPort:
		// check if port number satisfies its ipset's requirement of port range
		if set == nil {
			return false, fmt.Errorf("unable to reference ip set where the entry %v exists", e)
		}
		begin, end, err := parsePortRange(set.PortRange)
		if err != nil {
			return false, fmt.Errorf("failed to parse set %v port range %s for ipset %v, error: %v", set, set.PortRange, set, err)
		}
		if e.Port < begin || e.Port > end {
			return false, fmt.Errorf("entry %v port number %d is not in the port range %s of its ipset %v", e, e.Port, set.PortRange, set)
		}
	case HashIP:
		if ip := net.ParseIP(e.IP); ip == nil {
			err := fmt.Errorf("%s is not a valid textual representation of an IP address", e.IP)
			return false, fmt.Errorf("failed to parse entry %v ip %v for ipset %v, error: %v", e, e.IP, set, err)
		}
	case HashNet:
		if _, _, err := net.ParseCIDR(e.Net); err != nil {
			return false, fmt.Errorf("failed to parse entry %v net %v for ipset %v, error: %v", e, e.Net, set, err)
		}
...
func (e *Entry) checkIPandProtocol(set *IPSet) (bool, error) {
	// set default protocol to tcp if empty
	if len(e.Protocol) == 0 {
		e.Protocol = ProtocolTCP
	} else if ok, err := validateProtocol(e.Protocol); !ok {
		return false, err
	}

	if net.ParseIP(e.IP) == nil {
		return false, fmt.Errorf("error parsing entry %v ip address %v for ipset %v", e, e.IP, set)
	}
...
func (runner *runner) CreateSet(set *IPSet, ignoreExistErr bool) error {
	...
	if !valid {
		return fmt.Errorf("error creating ipset since it's invalid: %v", err)
	}
...
// checks if port range is valid. The beginning port number is not necessarily less than
...
func validateIPSetType(set Type) (bool, error) {
	for _, valid := range ValidIPSetTypes {
		if set == valid {
			return true, nil
		}
	}
	return false, fmt.Errorf("currently supported ipset types are: %v, %s is not supported", ValidIPSetTypes, set)
}

// checks if given hash family is supported in ipset
func validateHashFamily(family string) (bool, error) {
	if family == ProtocolFamilyIPV4 || family == ProtocolFamilyIPV6 {
		return true, nil
	}
	return false, fmt.Errorf("currently supported ip set hash families are: [%s, %s], %s is not supported", ProtocolFamilyIPV4, ProtocolFamilyIPV6, family)
}
...
func validateProtocol(protocol string) (bool, error) {
	if protocol == ProtocolTCP || protocol == ProtocolUDP || protocol == ProtocolSCTP {
		return true, nil
	}
	return false, fmt.Errorf("invalid entry's protocol: %s, supported protocols are [%s, %s, %s]", protocol, ProtocolTCP, ProtocolUDP, ProtocolSCTP)
}

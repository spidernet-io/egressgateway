...
func (r *endpointReconciler) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	log := r.log.With(
		...
		zap.String("kind", "EgressGatewayPolicy"),
	)
	...
	slicesToUpdate := make([]egressv1.EgressEndpointSlice, 0)
	slicesToCreate := make([]egressv1.EgressEndpointSlice, 0)
	slicesToDelete := make([]egressv1.EgressEndpointSlice, 0)
	slicesNotChange := make([]egressv1.EgressEndpointSlice, 0)
...
func newEndpointSlice(policy *egressv1.EgressGatewayPolicy) *egressv1.EgressEndpointSlice {
	...
	return &egressv1.EgressEndpointSlice{
...
func newEndpoint(pod corev1.Pod) *egressv1.EgressEndpoint {
	...
	return &egressv1.EgressEndpoint{
...
func listPodsByPolicy(ctx context.Context, cli client.Client, policy *egressv1.EgressGatewayPolicy) (*corev1.PodList, error) {
...
func listEndpointSlices(ctx context.Context, cli client.Client, namespace, policyName string) (*egressv1.EgressEndpointSliceList, error) {
	slices := new(egressv1.EgressEndpointSliceList)
...
func newEgressEndpointSliceController(mgr manager.Manager, log *zap.Logger, cfg *config.Config) error {
	...
	err = c.Watch(&source.Kind{Type: &egressv1.EgressGatewayPolicy{}}, &handler.EnqueueRequestForObject{})
	if err != nil {
		return fmt.Errorf("failed to watch EgressGatewayPolicy: %v", err)
	}

	err = c.Watch(&source.Kind{Type: &egressv1.EgressEndpointSlice{}}, &handler.EnqueueRequestForOwner{
		OwnerType:    &egressv1.EgressGatewayPolicy{},
		IsController: true,
	})

...
func GenIPPools(ctx context.Context, cli client.Client, egressConfig econfig.FileConfig, num int64, increase uint8) (egressv1.Ippools, error) {
	res := egressv1.Ippools{}
	ipv4, ipv6, err := getRandomEgressIPByNode(ctx, cli, increase)
	if err != nil {
		return res, err
	}
	if len(ipv4) != 0 && egressConfig.EnableIPv4 {
	...
	if len(ipv6) != 0 && egressConfig.EnableIPv6 {

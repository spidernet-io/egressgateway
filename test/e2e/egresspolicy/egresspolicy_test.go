...
var _ = Describe("Egresspolicy", func() {
	Context("test egressGatewayPolicy", Label("P00001"), func() {
		...
		BeforeEach(func() {
			...
			if testV4 {
				serverIPv4Netb, err := common.GetHostIPV4Net(time.Second * 10)
				Expect(err).NotTo(HaveOccurred())
				dst = append(dst, string(serverIPv4Netb))
				GinkgoWriter.Printf("dst: %v\n", dst)

				notDst = []string{"1.0.0.0/32"}

				serverIpv4b, err := common.GetHostIPV4(time.Second * 10)
				Expect(err).NotTo(HaveOccurred())
			...
			if testV6 {
				serverIPv6Netb, err := common.GetHostIPV6Net(time.Second * 10)
				Expect(err).NotTo(HaveOccurred())
				dst = append(dst, string(serverIPv6Netb))
				GinkgoWriter.Printf("dst: %v\n", dst)

				notDst = []string{"ffff::/128"}

				serverIpv6b, err := common.GetHostIPV6(time.Second * 10)
				Expect(err).NotTo(HaveOccurred())
		...
		It("operate egressgatewaypolicy", Label("P00001", "P00002", "P00003"), func() {
			...
			// sever status
			stats, err := tools.GetNetStats(time.Second * 30)
			Expect(err).NotTo(HaveOccurred())
			GinkgoWriter.Printf("server status: %s\n", stats)

			// kernel parameters
			params, err := tools.GetKernelParams(time.Second * 30)
			Expect(err).NotTo(HaveOccurred())
			GinkgoWriter.Printf("kernel parameters: %s\n", params)

...
var _ = Describe("EgressPolicy", func() {
	Context("Test egressGatewayPolicy", Label("EgressPolicy"), func() {
		...
		DescribeTable("Test policy", Serial, func(isGlobal bool, createPolicy func() error) {
			...
			checkEip(podBList, v4Eip, v6Eip, serverIPv4A, serverIPv6A, true, time.Second*5)

			// check eip in podA, we expect pods export ip is not eip
			GinkgoWriter.Printf("Check export ip in dsA: %s pods\n", dsNameA)
			checkEip(podAList, v4Eip, v6Eip, serverIPv4A, serverIPv6A, false, time.Second*5)
			...
			if isGlobal {
				// update egressPolicy
				updatePolicy(egressPolicyName, egressClusterPolicy, nil, nil)
			} else {
				// update egressClusterPolicy
				updatePolicy(egressPolicyName, egressPolicy, nil, nil)
			}
			...
			Entry("When namespace-level", false, Label("P00011"), func() error {
				GinkgoWriter.Printf("GenerateEgressPolicyYaml: %s\n", egressPolicyName)
				return common.CreateEgressPolicy(f, common.GenerateEgressPolicyYaml(egressPolicyName, egressGatewayName, common.NSDefault, emptyEgressIP, podALabel, nil, nil))
			}),
		)
	})
})
...
func checkEip(podList *corev1.PodList, v4Eip, v6Eip, serverIPv4, serverIPv6 string, expect bool, timeout time.Duration) {
